...
## 크롤링 함수 구현하기 ##
def crawlig(day,month,year):
    url = "http://welfoodstory.azurewebsites.net/?category=2%EC%BA%A0%ED%8D%BC%EC%8A%A4-3&date=" + str(year) + "-" + month + "-" + day
    print(url)
...
def app_mentioned(event_data):
    ...
    button_actions = ActionsBlock(
        elements=[
            ButtonElement(
                text="어제",
                action_id="yesterday", value="-1"
            ),
            ButtonElement(
                text="오늘", style="danger",
                action_id="today", value="0"
            ),
            ButtonElement(
                text="내일",
                action_id="tomorrow", value="1"
            ),
            ButtonElement(
                text="모레", style="primary",
                action_id="theDayAfterTomorrow", value="2"
            ),
        ]
    )
...
def on_button_click():
    ...
    print(type(weekday) , " : weekday :", weekday)
    ## 주말 처리 ##
    if weekday >= 5 :
        # 블록 처리 ##
        my_blocks = []
        contentBlockTmp = SectionBlock(
            text="` *     주말은 쏘리이이이이이이이이이이ㅣ잉      * ` "
        )
        my_blocks.append(contentBlockTmp)
    ...
    ## 날짜별 데이터 파싱 ##
    objectList = []
    objectList = crawlig(dateData,monthStr,year)
